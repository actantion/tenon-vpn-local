...
static int use_smart_route __attribute__((unused)) = 0;
static unsigned int route_ip __attribute__((unused)) = 0;
static unsigned short route_port __attribute__((unused)) = 0;
static uint32_t vpn_ip __attribute__((unused)) = 0;
static uint16_t vpn_port __attribute__((unused)) = 0;
static char* seckey __attribute__((unused)) = NULL;
static char* pubkey __attribute__((unused)) = NULL;
static char* enc_method __attribute__((unused)) = NULL;
    ...
    if (!remote->send_ctx->connected) {
        ...
        if (!fast_open || remote->direct) {
            ...
            if (s == 0) {
                ...
                memcpy(server_head + start_pos + 67, enc_method, m_len);
                int send_len = start_pos + 66 + 1 + m_len;
                char* old_data = malloc(remote->buf->len);
...
{
    LOGI("Dddddddddddsdddddddddddd111111");
    int i, c;
    int pid_flags    = 0;
    int mtu          = 0;
    int mptcp        = 0;
    ...
    while ((c = getopt_long(argc, argv, "f:s:p:l:k:t:m:i:c:b:a:n:huUv6A",
        ...
        LOGI("Dddddddddddsdddddddddddd22222");
    if (opterr) {
        usage();
        exit(EXIT_FAILURE);
    }
    ...
    if (conf_path != NULL) {
        ...
        LOGI("Dddddddddddsdddddddddddd4444");
    if (remote_num == 0) {
    ...
    // Setup keys
    LOGI("initializing ciphers... %s, %s", seckey, enc_method);
    crypto = crypto_init(seckey, key, enc_method);
    if (crypto == NULL)
    ...
    for (i = 0; i < remote_num; i++) {
        ...
        char str_port[16] = { 0 };
        if (use_smart_route != 0) {
            inet_ntop(AF_INET, (const void *)(&route_ip), str_host, INET_ADDRSTRLEN);
            snprintf(str_port, 16, "%d", route_port);
        } else {
            inet_ntop(AF_INET, (const void *)(&vpn_ip), str_host, INET_ADDRSTRLEN);
            snprintf(str_port, 16, "%d", vpn_port);
        }
